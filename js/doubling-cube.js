...
export function handleCubeClick(player) {
  ...
  if (state.gamePhase === "opening_roll") {
    logStatus(
      "Doubling cube is disabled during the opening roll.",
      DISPLAY_TIME,
    );
    return;
  }
  ...
  if (state.cubeOwner !== "center" && state.cubeOwner !== player) {
    logStatus(
      `You cannot double - ${state.cubeOwner} owns the doubling cube!`,
      DISPLAY_TIME,
    );
    return;
  }
  ...
  clearStatusQueue();
  logStatus(
    `${player} offered to double the cube to ${targetValue}. Does ${respondingPlayer} accept?`,
  );
...
export function resolveCubeOffer(accepted, targetValue) {
  ...
  if (accepted) {
    ...
    if (currentZone) {
      currentZone.style.display = ""; // Removed 'flex' so CSS can control
    }
